            ...
            <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto px-6 py-4 space-y-5">
              ...
              <div>
                ...
                <p className="text-xs text-slate-500 mb-3">
                  The first image is the <span className="font-semibold">front view</span> and the second is the{' '}
                  <span className="font-semibold">back view</span>. Products with two images get swipe and arrow
                  navigation on the shop cards.
                </p>
                ...
                {formData.images.length > 0 && (
                  <div className="grid grid-cols-4 gap-2">
                    {formData.images.map((img, idx) => (
                      <div key={idx} className="relative group aspect-square rounded-lg overflow-hidden bg-slate-100 ring-1 ring-slate-200">
                        <img src={img?.url || img} alt="" className="w-full h-full object-cover" />
                        {idx === 0 ? (
                          <span className="absolute bottom-1 left-1 px-1.5 py-0.5 rounded bg-primary-500 text-[10px] font-bold text-white">
                            FRONT
                          </span>
                        ) : (
                          <button
                            type="button"
                            onClick={() => makeFrontImage(idx)}
                            className="absolute bottom-1 left-1 px-1.5 py-0.5 rounded bg-black/60 text-[10px] font-semibold text-white hover:bg-black/80 transition-colors"
                          >
                            Set front
                          </button>
                        )}
